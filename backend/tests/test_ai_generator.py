...
def _make_generator():
    ...
    with patch("ai_generator.AnthropicFoundry") as MockClient:
...
def test_tool_use_dispatches_to_tool_manager():
    ...
    first = make_response(
        "tool_use",
        [make_tool_use_block("search_course_content", {"query": "what is mcp"}, "tu_1")],
    )
    second = make_response("end_turn", [make_text_block("MCP is a protocol.")])
    client.messages.create.side_effect = [first, second]

    tool_manager = MagicMock()
    tool_manager.execute_tool.return_value = "[MCP Basics - Lesson 1]\nMCP is a protocol."

    tools = [{"name": "search_course_content"}]
    out = gen.generate_response(query="what is mcp", tools=tools, tool_manager=tool_manager)
...
def test_two_round_sequential_flow():
    ...
    second = make_response(
        "tool_use",
        [make_tool_use_block("search_course_content", {"query": "Lesson 4 Topic"}, "t2")],
    )
    ...
    out = gen.generate_response(query="find a course like lesson 4 of X",
...
def test_max_rounds_caps_tool_calls_then_forces_text_answer():
    ...
    out = gen.generate_response(query="q", tools=[{"name": "search_course_content"}],
                                tool_manager=tool_manager)
...
def test_tool_error_string_flows_back_normally():
    ...
    out = gen.generate_response(query="outline of Z", tools=[{"name": "get_course_outline"}],
                                tool_manager=tool_manager)
